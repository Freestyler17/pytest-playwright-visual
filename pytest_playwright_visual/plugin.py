...
def assert_snapshot(pytestconfig: Any, request: Any, browser_name: str) -> Callable:
    ...
    def compare(img: bytes, *, threshold: float = 0.1, name=f'{test_name}.png', fail_fast=False) -> None:
        ...
        repo_base_path = Path(__file__).parent.parent  # Wskazuje na główny katalog repo
        filepath = (
            repo_base_path  
            / 'test_snapshots'  
            / test_file_name
        ...
        results_dir_name = repo_base_path / "test_snapshots_failures"
        ...
        if mismatch == 0:
            ...
            # Attach images to Allure
            import allure
            allure.attach.file(f'{test_results_dir}/Diff_{name}', name='Diff', attachment_type=allure.attachment_type.PNG)
            allure.attach.file(f'{test_results_dir}/Actual_{name}', name='Actual', attachment_type=allure.attachment_type.PNG)
            allure.attach.file(f'{test_results_dir}/Expected_{name}', name='Expected', attachment_type=allure.attachment_type.PNG)
